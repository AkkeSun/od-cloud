...
class SubscriptionRepository {
    ...
    /**
     * 대상 로우에 비관적 쓰기 락(PESSIMISTIC_WRITE)을 걸고 조회한다. 동일 구독에 대한 동시 플랜 변경 요청이 서로 다른 트랜잭션(커넥션)에서 들어올 때,
     * 뒤에 도착한 트랜잭션이 앞선 트랜잭션의 커밋을 기다리도록 강제하여 상태를 직렬화한다. (같은 영속성 컨텍스트 내 재조회로는 다른 트랜잭션의 커밋을 감지할 수 없으므로,
    ...
    List<SubscriptionDetail> findActiveByGroupIds(List<Long> groupIds) {
        ...
        return queryFactory
            ...
            .leftJoin(subscriptionEntity).on(subscriptionEntity.productId.eq(productEntity.id)
                .and(groupMatch)
                .and(subscriptionEntity.status.in("ACTIVE", "EXP_PENDING", "DOWN_PENDING")))
