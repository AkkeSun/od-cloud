...
class ApiInfoStorageAdapterTest extends IntegrationTestSupport {
    ...
    class Describe_findAll {
        ...
        @Test
        @DisplayName("[success] 캐시된 데이터를 반환한다")
        void success_cached() {
            // given
            entityManager.persist(ApiInfoEntity.builder()
                .domain("account")
            ...
            // when - 첫 번째 조회
            List<ApiInfo> firstResult = adapter.findAll();

            // 새로운 데이터 추가 (하지만 캐시 때문에 반영되지 않음)
            entityManager.persist(ApiInfoEntity.builder()
                .domain("group")
                .httpMethod("POST")
                .uriPattern("/api/groups")
            ...
            // 두 번째 조회 (캐시된 데이터)
            List<ApiInfo> secondResult = adapter.findAll();

            // then
            assertThat(firstResult).hasSize(1);
            assertThat(secondResult).hasSize(1);  // 캐시된 데이터이므로 여전히 1개
        }

        @Test
        @DisplayName("[success] 캐시 삭제 후 새로운 데이터를 조회한다")
        void success_afterCacheClear() {
            // given
            entityManager.persist(ApiInfoEntity.builder()
                .domain("account")
                .httpMethod("GET")
            ...
            // then
            assertThat(firstResult).hasSize(1);
            assertThat(secondResult).hasSize(2);  // 캐시 삭제 후 새로운 데이터 반영
        }
    ...
    @Nested
    @DisplayName("[deleteApiInfoCache] API 정보 캐시를 삭제하는 메소드")
    class Describe_deleteApiInfoCache {

        @Test
        @DisplayName("[success] 캐시를 삭제한다")
        void success() {
            // given
            entityManager.persist(ApiInfoEntity.builder()
                .domain("account")
            ...
            // 첫 번째 조회로 캐시 생성
            List<ApiInfo> beforeClear = adapter.findAll();
            ...
            // when
            adapter.deleteApiInfoCache();
            List<ApiInfo> afterClear = adapter.findAll();

            // then
            assertThat(beforeClear).hasSize(1);
            assertThat(afterClear).hasSize(2);
        }

        @Test
        @DisplayName("[success] 캐시가 없어도 정상 동작한다")
        void success_noCacheExists() {
            // when & then - 예외 없이 정상 동작해야 함
            adapter.deleteApiInfoCache();
        }

        @Test
        @DisplayName("[success] 캐시 삭제 후 다시 조회하면 새로운 캐시가 생성된다")
        void success_rebuildCache() {
            // given
            entityManager.persist(ApiInfoEntity.builder()
                .domain("account")
                .httpMethod("GET")
            ...
            // then
            assertThat(firstResult).hasSize(1);
            assertThat(secondResult).hasSize(1);  // 캐시된 데이터
        }
    }
}
