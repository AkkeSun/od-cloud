...
class FolderStorageAdapterTest extends IntegrationTestSupport {
    ...
    class Describe_save {
        ...
        void success_rootFolder() {
            ...
            var savedFolders = entityManager
                .createQuery("SELECT f FROM FolderEntity f WHERE f.groupId = :groupId", FolderEntity.class)
        ...
        void success_subFolder() {
            ...
            // then
            var savedFolders = entityManager
                .createQuery("SELECT f FROM FolderEntity f WHERE f.parentId = :parentId", FolderEntity.class)
        ...
        void success_update() {
            ...
            // then
            FolderEntity savedEntity = entityManager.find(FolderEntity.class, existingFolder.getId());
        ...
        void success_multipleSubFolders() {
            ...
            // then
            var savedFolders = entityManager
                .createQuery("SELECT f FROM FolderEntity f WHERE f.parentId = :parentId ORDER BY f.name", FolderEntity.class)
                .setParameter("parentId", parentFolder.getId())
                .getResultList();
        ...
        void success_variousAccessLevels() {
            ...
            // then
            var savedFolders = entityManager
                .createQuery("SELECT f FROM FolderEntity f WHERE f.groupId = :groupId ORDER BY f.accessLevel", FolderEntity.class)
                .setParameter("groupId", "test-group")
                .getResultList();
    ...
    class Describe_existsSameFolderName {
        ...
        @Test
        @DisplayName("[success] 루트 폴더에서 이름 중복을 확인한다")
        void success_rootLevel() {
            // given
            LocalDateTime now = LocalDateTime.now();

            // 루트 폴더 생성 (parentId가 null)
            FolderEntity rootFolder = FolderEntity.builder()
                .parentId(null)
                .groupId("test-group")
                .name("루트 폴더")
                .owner("owner@example.com")
            ...
            // when
            boolean result = adapter.existsSameFolderName(null, "루트 폴더");
