...
class UpdateFileControllerDocsTest extends RestDocsSupport {
    ...
    class Describe_update {
        ...
        void error_unauthorized() throws Exception {
            ...
            // when then
            performErrorDocument(fileId, request, status().isUnauthorized(), "인증 토큰 미입력 혹은 만료된 토큰 입력",
                authorization);
        }

        @Test
        @DisplayName("[success] 파일명만 수정한다")
        void success_updateFileNameOnly() throws Exception {
            // given
            Long fileId = 1L;
            ...
            // when & then
            performDocument(fileId, request, status().isOk(), "파일명만 수정", "success", authorization,
        ...
        void success_updateFolderOnly() throws Exception {
            // given
            Long fileId = 1L;
            String authorization = "Bearer test";
            UpdateFileRequest request = UpdateFileRequest.builder()
            ...
            // when & then
            performDocument(fileId, request, status().isOk(), "폴더만 변경", "success", authorization,
                fieldWithPath("httpStatus")
                    .type(JsonFieldType.NUMBER).description("상태 코드"),
                fieldWithPath("message")
                    .type(JsonFieldType.STRING).description("상태 메시지"),
        ...
        void success_updateBoth() throws Exception {
            ...
            // when & then
            performDocument(fileId, request, status().isOk(), "파일명과 폴더 동시 변경", "success", authorization,
                fieldWithPath("httpStatus")
                    .type(JsonFieldType.NUMBER).description("상태 코드"),
                fieldWithPath("message")
                    .type(JsonFieldType.STRING).description("상태 메시지"),
        ...
        void error_fileNotFound() throws Exception {
            ...
            // when & then
            performErrorDocument(fileId, request, status().isInternalServerError(), "파일 없음", authorization);
        }
        ...
        void error_duplicateFileName() throws Exception {
            ...
            // when & then
            performErrorDocument(fileId, request, status().isInternalServerError(), "중복된 파일명", authorization);
        }
        ...
        void error_forbiddenAccess() throws Exception {
            ...
            // when & then
            performErrorDocument(fileId, request, status().isInternalServerError(), "접근 권한 없음", authorization);
        }
        ...
        void error_targetFolderNotFound() throws Exception {
            ...
            // when & then
            performErrorDocument(fileId, request, status().isInternalServerError(), "대상 폴더 없음", authorization);
        }
    }
        ...
        mockMvc.perform(
            ...
            .andDo(document(String.format("[%s] %s", apiName, docIdentifier),
                    ...
                    resource(ResourceSnippetParameters.builder()
                        ...
                        .requestFields(
                            fieldWithPath("fileName").type(JsonFieldType.STRING).description("변경할 파일명 (옵션)").optional(),
                            fieldWithPath("folderId").type(JsonFieldType.NUMBER).description("이동할 폴더 ID (옵션)").optional()
                        )
