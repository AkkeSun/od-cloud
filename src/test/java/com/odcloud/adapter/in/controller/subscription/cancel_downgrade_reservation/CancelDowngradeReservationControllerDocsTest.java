...
class CancelDowngradeReservationControllerDocsTest extends RestDocsSupport {

    private final CancelDowngradeReservationUseCase useCase =
        mock(CancelDowngradeReservationUseCase.class);
    private final String apiName = "다운그레이드 예약 취소 API";

    @Override
    protected Object initController() {
        return new CancelDowngradeReservationController(useCase);
    }

    @Nested
    @DisplayName("[cancel] 다운그레이드 예약을 취소하는 API")
    class Describe_cancel {

        @Test
        @DisplayName("[success] 다운그레이드 예약을 취소한다")
        void success() throws Exception {
            // given
            given(useCase.cancel(any()))
                .willReturn(CancelDowngradeReservationResponse.ofSuccess());
        ...
        @Test
        @DisplayName("[error] DOWN_PENDING 상태가 아닌 구독이면 에러를 반환한다")
        void error_invalidStatus() throws Exception {
            // given
            given(useCase.cancel(any())).willThrow(new CustomBusinessException(
                ErrorCode.Business_INVALID_SUBSCRIPTION_STATUS_FOR_DOWNGRADE_CANCEL));

            // when & then
            performErrorDocument(1L, "Bearer test",
                status().isInternalServerError(), "DOWN_PENDING 상태가 아닌 구독");
        }
    }
    ...
    private void performDocument(
        ...
        mockMvc.perform(patch("/subscriptions/{subscriptionId}/downgrade-cancellation", subscriptionId)
            ...
            .andDo(document(String.format("[%s] %s", apiName, docIdentifier),
                ...
                resource(ResourceSnippetParameters.builder()
                    .tag("Subscription")
                    .summary("다운그레이드 예약 취소 API")
                    .description("로그인 사용자가 플랜 다운그레이드 예약(DOWN_PENDING)을 취소하고 "
                        + "현재 구독을 계속 ACTIVE로 유지하는 API 입니다.<br><br>"
                        + "- 구독 상태를 DOWN_PENDING 에서 ACTIVE 로 전환합니다.<br>"
                        + "- 같은 그룹에 예약되어 있던 다운그레이드 대상(PENDING) 구독을 삭제합니다.<br>"
                        + "- 구독의 buyerId 와 로그인 사용자가 다르면 접근 권한 예외가 발생합니다.<br>"
                        + "- 구독 상태가 DOWN_PENDING 이 아니면 예외가 발생합니다.")
                    .pathParameters(
                        parameterWithName("subscriptionId").description("다운그레이드 예약을 취소할 구독 ID")
                    )
