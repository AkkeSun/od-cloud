...
class DownloadFilesControllerDocsTest extends RestDocsSupport {
    ...
    class Describe_downloadFiles {
        ...
        void error_emptyFileIds() throws Exception {
            ...
            // when & then
            performErrorDocument(status().isBadRequest(), new ArrayList<>(), "빈 파일 ID 목록", authorization);
        }
        ...
        void error_fileNotFound() throws Exception {
            ...
            // when & then
            performErrorDocument(status().isInternalServerError(), List.of(1L,2L), "파일 조회 실패", authorization);
        }
        ...
        void error_downloadError() throws Exception {
            ...
            // when & then
            performErrorDocument(status().isInternalServerError(), List.of(1L, 2L),"파일 다운로드 에러", authorization);
        }
    }
