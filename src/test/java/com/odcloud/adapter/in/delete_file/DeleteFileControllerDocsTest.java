...
class DeleteFileControllerDocsTest extends RestDocsSupport {
    ...
    class Describe_delete {
        ...
        void success_multipleFiles() throws Exception {
            ...
            // when & then
            performDocument(status().isOk(), "success_multiple", "success", authorization, requestBody,
                fieldWithPath("httpStatus")
                    .type(JsonFieldType.NUMBER).description("상태 코드"),
                fieldWithPath("message")
                    .type(JsonFieldType.STRING).description("상태 메시지"),
        ...
        void partialSuccess() throws Exception {
            ...
            // when & then
            performDocument(status().isOk(), "partial_success", "partial_success", authorization, requestBody,
                fieldWithPath("httpStatus")
                    .type(JsonFieldType.NUMBER).description("상태 코드"),
                fieldWithPath("message")
                    .type(JsonFieldType.STRING).description("상태 메시지"),
        ...
        void error_fileDoesNotExist() throws Exception {
            ...
            // when & then
            performDocument(status().isOk(), "error_file_not_found", "error", authorization, requestBody,
                fieldWithPath("httpStatus")
                    .type(JsonFieldType.NUMBER).description("상태 코드"),
                fieldWithPath("message")
                    .type(JsonFieldType.STRING).description("상태 메시지"),
        ...
        mockMvc.perform(
            ...
            .andDo(document(String.format("[%s] %s", apiName, docIdentifier),
                    ...
                    resource(ResourceSnippetParameters.builder()
                        ...
                        .requestFields(
                            fieldWithPath("fileIds").type(JsonFieldType.ARRAY).description("삭제할 파일 ID 목록")
                        )
