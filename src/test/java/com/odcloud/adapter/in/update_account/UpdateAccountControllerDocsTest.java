...
class UpdateAccountControllerDocsTest extends RestDocsSupport {
    ...
    class Describe_updateAccount {
        ...
        void success() throws Exception {
            ...
            // when & then
            performDocument(authorization, nickname, pictureFile, status().isOk(), "success", "success",
        ...
        void success_pictureOnly() throws Exception {
            ...
            // when & then
            performDocument(authorization, null, pictureFile, status().isOk(), "프로필 사진만 수정", "success",
                fieldWithPath("httpStatus").type(JsonFieldType.NUMBER)
                    .description("상태 코드"),
                fieldWithPath("message").type(JsonFieldType.STRING)
                    .description("상태 메시지"),
