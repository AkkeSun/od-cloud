...
class TextUtilTest {
    ...
    class Describe_truncateTextLimit {
        ...
        @Test
        @DisplayName("[success] 이모지를 포함한 긴 텍스트를 잘라낸다")
        void success_truncateTextWithEmoji() {
            // given
            StringBuilder sb = new StringBuilder();
        ...
        @Test
        @DisplayName("[success] 다양한 바이트 크기의 문자가 섞인 텍스트를 안전하게 잘라낸다")
        void success_mixedByteCharacters() {
            // given
            StringBuilder sb = new StringBuilder();
            ...
            String mixedText = sb.toString();

            // when
            String result = TextUtil.truncateTextLimit(mixedText);

            // then
            int resultByteSize = result.getBytes(StandardCharsets.UTF_8).length;
            assertThat(resultByteSize).isLessThanOrEqualTo(65535);
            assertThat(result.length()).isLessThan(mixedText.length());
        }
