...
class GroupAccountTest {
    ...
    class Describe_builder {
        ...
        @Test
        @DisplayName("[success] Builder로 GroupAccount를 생성한다")
        void success() {
            // given
        ...
        @Test
        @DisplayName("[success] null 값으로 GroupAccount를 생성한다")
        ...
        @Test
        @DisplayName("[success] 다양한 status로 GroupAccount를 생성한다")
        void success_variousStatuses() {
            // when
            GroupAccount pending = GroupAccount.builder().status("PENDING").build();
            GroupAccount active = GroupAccount.builder().status("ACTIVE").build();
            GroupAccount approved = GroupAccount.builder().status("APPROVED").build();
    ...
    @Nested
    @DisplayName("[of] Group과 Account로부터 GroupAccount를 생성하는 정적 팩토리 메서드")
    class Describe_of {

        @Test
        @DisplayName("[success] Group과 Account로부터 GroupAccount를 생성한다")
        void success() {
            // given
            ...
            // when
            GroupAccount groupAccount = GroupAccount.of(group, account);
            ...
            assertThat(groupAccount.getGroupId()).isEqualTo("group-123");
            assertThat(groupAccount.getAccountId()).isEqualTo(100L);
            assertThat(groupAccount.getStatus()).isEqualTo("PENDING");
            assertThat(groupAccount.getRegDt()).isAfter(before);
            assertThat(groupAccount.getRegDt()).isBefore(after);
        }

        @Test
        @DisplayName("[success] null 값을 가진 Group과 Account로부터 GroupAccount를 생성한다")
        void success_nullValues() {
            // given
            Group group = Group.builder()
                .id(null)
    ...
    @Nested
    @DisplayName("[isApproved] status가 APPROVED인지 확인하는 메서드")
    class Describe_isApproved {

        @Test
        @DisplayName("[success] status가 APPROVED이면 true를 반환한다")
        void success_approved() {
            ...
            // when
            boolean result = groupAccount.isApproved();
        ...
        @Test
        @DisplayName("[success] status가 ACTIVE이면 false를 반환한다")
        void success_active() {
            // given
            GroupAccount groupAccount = GroupAccount.builder()
                .status("ACTIVE")
