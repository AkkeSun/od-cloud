...
class ApiCallLogTest {
    ...
    class Describe_builder {
        ...
        @Test
        @DisplayName("[success] Builder로 ApiCallLog를 생성한다")
        void success() {
            // given
        ...
        @Test
        @DisplayName("[success] null 값으로 ApiCallLog를 생성한다")
        void success_nullValues() {
            ...
            // then
            assertThat(log.getAccountInfo()).isNull();
            assertThat(log.getApiId()).isNull();
            assertThat(log.getUri()).isNull();
            assertThat(log.getHttpMethod()).isNull();
            assertThat(log.getRequestPathParam()).isNull();
            assertThat(log.getRequestParam()).isNull();
            assertThat(log.getRequestBody()).isNull();
            assertThat(log.getResponseBody()).isNull();
            assertThat(log.getHttpStatus()).isNull();
            assertThat(log.getErrorCode()).isNull();
            assertThat(log.getRegDt()).isNull();
        }

        @Test
        @DisplayName("[success] 에러 코드를 포함한 ApiCallLog를 생성한다")
        void success_withErrorCode() {
            // when
            ApiCallLog log = ApiCallLog.builder()
                .uri("/api/users/123")
                .httpMethod("GET")
    ...
    class Describe_getResponseLog {
        ...
        void success_errorResponse() {
            ...
            assertThat(result).contains("POST");
            assertThat(result).contains("/api/users");
            assertThat(result).contains("{\"error\":\"INVALID_REQUEST\",\"message\":\"잘못된 요청입니다\"}");
        }
    }
